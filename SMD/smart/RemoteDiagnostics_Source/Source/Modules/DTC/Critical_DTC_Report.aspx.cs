...
public partial class Modules_DTC_Critical_DTC_Report : System.Web.UI.Page
{
    ...
    public string getCriticalDTCCount(object DTCCode, object VINNumber)
    {
        string res = string.Empty;
        var crtDTCCode = string.Empty;
        var crtVINNumber = string.Empty;
        if (DTCCode != DBNull.Value && VINNumber != DBNull.Value)
        {
            crtDTCCode = Convert.ToString(DTCCode);
            crtVINNumber = Convert.ToString(VINNumber);

            using (miniSmartDataContext db = new miniSmartDataContext())
            {
                var qry = (from m in db.tab_CA_EMS_MASTER_INFOs
                           join d in db.tab_CA_EMS_DTC_INFOs on m.freeze_MASTER_ID equals d.freeze_MASTER_ID
                           where m.freeze_VIN_NUMBER == crtVINNumber && d.freeze_DTC_CODE == crtDTCCode
                           select m).Count();
                if (qry > 0)
                {
                    res = Convert.ToString(qry);
                }
            }
        }
        return res;
    ...
    private void CreateExcel()
    {
        ...
        {
            xlPack.Workbook.Properties.Title = "Remote Diagnostic Tool - Critical DTC Report Card";

            //Worksheet
            xlPack.Workbook.Worksheets.Add("Critical DTC Report Card");
            ExcelWorksheet xlSheet = xlPack.Workbook.Worksheets[1];
            xlSheet.Name = "Critical DTC Report Card";
            xlSheet.Cells["A:XFD"].Style.Font.Name = "Arial";

            DataTable ObjDT = GetDataTable(_sStartDate, _sEndDate);

            //Title 
            var cell = xlSheet.Cells[1, 1, 1, 13];
            cell.Value = "Remote Diagnostic Tool - Critical DTC Report Card";
            cell.Merge = true;
            ...
            cell.Value = "Push Notification Status";
            cell.Merge = true;
            cell.Style.Font.Bold = true;
            cell.Style.Font.Size = 10;
            cell.Style.Font.Name = "Calibri";
            ...
            cell.Value = "Notification Counter";
            cell.Style.Font.Bold = true;
            cell.Merge = true;
            cell.Style.Font.Size = 10;
            cell.Style.Font.Name = "Calibri";
            ...
            foreach (DataRow row in ObjDT.Rows)
            {
                ...
                DataRow(xlSheet, RowIndex, 3, subRowCount, 3, false, "Vehicle Name", row["crt_PLATFORM_NAME"]);
                DataRow(xlSheet, RowIndex, 4, subRowCount, 4, false, "ECU Name", row["crt_ECU_NAME"]);
                DataRow(xlSheet, RowIndex, 5, subRowCount, 5, false, "DTC Code", row["crt_DTC_CODE"]);
                DataRow(xlSheet, RowIndex, 6, subRowCount, 6, false, "Occurrence Counter", getCriticalDTCCount(row["crt_DTC_CODE"], row["freeze_VIN_NUMBER"]));
                DataRow(xlSheet, RowIndex, 7, subRowCount, 7, false, "Push Notification Status", row["freeze_PUSH_NOTIFY"]);
                DataRow(xlSheet, RowIndex, 8, subRowCount, 8, false, "Notification Counter", row["freeze_NOTIFY_COUNTER"]);
                ...
                if (dtSub.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow rowSub in dtSub.Rows)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                DataRow(xlSheet, vehicleMergeIndex, 9, vehicleMergeIndex + mergeRowIndex, 9, false, "DTC Code", rowSub["freeze_DTC_CODE"]);
                                DataRow(xlSheet, vehicleMergeIndex, 10, vehicleMergeIndex + mergeRowIndex, 10, false, "Description", rowSub["freeze_DTC_DESCRIPTION"]);
                                DataRow(xlSheet, vehicleMergeIndex, 11, vehicleMergeIndex + mergeRowIndex, 11, false, "Status", rowSub["freeze_DTC_STATUS"]);
                            }
                            else
                            {
                                DataRow(xlSheet, RowIndex, 9, RowIndex, 9, false, "DTC Code", rowSub["freeze_DTC_CODE"]);
                                DataRow(xlSheet, RowIndex, 10, RowIndex, 10, false, "Description", rowSub["freeze_DTC_DESCRIPTION"]);
                                DataRow(xlSheet, RowIndex, 11, RowIndex, 11, false, "Status", rowSub["freeze_DTC_STATUS"]);
                            }
                            mergeRowIndex = 0;
                        }
                        DataRow(xlSheet, RowIndex, 9, RowIndex, 9, false, "DTC Code", rowSub["freeze_DTC_CODE"]);
                        DataRow(xlSheet, RowIndex, 10, RowIndex, 10, false, "Description", rowSub["freeze_DTC_DESCRIPTION"]);
                        DataRow(xlSheet, RowIndex, 11, RowIndex, 11, false, "Status", rowSub["freeze_DTC_STATUS"]);
                        DataRow(xlSheet, RowIndex, 12, RowIndex, 12, false, "Signals", rowSub["freeze_SIGNAL"]);
                        DataRow(xlSheet, RowIndex, 13, RowIndex, 13, false, "Value", rowSub["freeze_VALUE"]);
                        RowIndex++;
                        if (Convert.ToString(rowSub["freeze_DTC_CODE"]) != (i < dtSub.Rows.Count ? Convert.ToString(dtSub.Rows[i]["freeze_DTC_CODE"]) : ""))
                        {
                ...
                {
                    DataRow(xlSheet, RowIndex, 9, RowIndex, 9, false, "DTC Code", "");
                    DataRow(xlSheet, RowIndex, 10, RowIndex, 10, false, "Description", "");
                    DataRow(xlSheet, RowIndex, 11, RowIndex, 11, false, "Status", "");
                    DataRow(xlSheet, RowIndex, 12, RowIndex, 12, false, "Signals", "");
                    DataRow(xlSheet, RowIndex, 13, RowIndex, 13, false, "Value", "");
                    RowIndex++;
                }
                Count++;
            }
            xlSheet.View.FreezePanes(6, 6);
            //Save
            Byte[] bin = xlPack.GetAsByteArray();
            string fileName = "Critical_DTC_Report_Card_" + string.Format("{0:yyyy}", DateTime.Now) + ".xlsx";
